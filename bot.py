...
QUESTIONS = [
    ...
    "🤝 *16/30* Тип клиентов: B2B, B2C или оба?",
    ...
    "🔗 *25/30* Ссылка на ваш LinkedIn или HH.ru (или напишите «нет»)?",
...
KB_DRIVING  = ReplyKeyboardMarkup([["Да, права и авто", "Есть права, нет авто", "Нет"]], resize_keyboard=True, one_time_keyboard=True)
KB_YESNO    = ReplyKeyboardMarkup([["Да", "Нет"]], resize_keyboard=True, one_time_keyboard=True)
KB_NONE     = ReplyKeyboardRemove()
 
KEYBOARDS = {15: KB_CLIENTS, 19: KB_SCHEDULE, 20: KB_ENGLISH, 21: KB_DRIVING, 22: KB_YESNO, 23: KB_YESNO}
...
def make_step(step_index):
    def handler(update: Update, context: CallbackContext):
        ...
        if text != "✅ Начать анкету":
            context.user_data.setdefault("answers", []).append(text)
        ...
        kb = KEYBOARDS.get(next_i, KB_NONE)
        update.message.reply_text(QUESTIONS[next_i], parse_mode="Markdown", reply_markup=kb)
        return next_i
    return handler
...
def finish(update: Update, context: CallbackContext):
    ...
    update.message.reply_text("⏳ Сохраняю анкету...", reply_markup=KB_NONE)
    ...
    if ADMIN_CHAT_ID:
        ...
        with open(pdf, "rb") as f:
            context.bot.send_document(ADMIN_CHAT_ID, f, filename=f"Анкета_{name}.pdf")
        with open(EXCEL_FILE, "rb") as f:
            context.bot.send_document(ADMIN_CHAT_ID, f, filename="Все_кандидаты.xlsx")
...
def cancel(update: Update, context: CallbackContext):
    update.message.reply_text("❌ Анкета отменена. Напишите /start чтобы начать заново.", reply_markup=KB_NONE)
    return ConversationHandler.END
...
def main():
    ...
    states = {i: [MessageHandler(Filters.text & ~Filters.command, make_step(i))] for i in range(30)}
